...
@section('title', 'Rekap Absensi | Peserta - SMART Setjen DPR RI')
@section('content')
<!-- Import Fonts & Icons -->
...
<style>
    ...
    /* === Dark mode: peserta-absensi konsisten === */
    html[data-skin="dark"] body,
    html[data-skin="dark"] .content-wrapper { background-color: #020617 !important; color: #ffffff !important; }
    ...
    html[data-skin="dark"] .breadcrumb-item { color: #fbbf24 !important; }
    html[data-skin="dark"] .modern-card { background: #0f172a !important; border-color: #1f2937 !important; }
    html[data-skin="dark"] .card-toolbar { background: #0f172a !important; border-color: #1f2937 !important; }
    html[data-skin="dark"] .card-toolbar .text-dark,
    html[data-skin="dark"] .filter-box,
    html[data-skin="dark"] .filter-section { background: #1e293b !important; border-color: #374151 !important; }
    html[data-skin="dark"] .search-container .input-group { background: #1e293b !important; border-color: #374151 !important; }
    html[data-skin="dark"] .search-container input { color: #ffffff !important; }
    html[data-skin="dark"] .form-control-custom { background: #1e293b !important; border-color: #374151 !important; color: #ffffff !important; }
    ...
    html[data-skin="dark"] .custom-table tbody tr { background: #0f172a !important; }
    html[data-skin="dark"] .custom-table tbody td { background: #0f172a !important; }
    html[data-skin="dark"] .footer-container,
    html[data-skin="dark"] .px-4.py-3[style*="background"] { background: #1e293b !important; border-color: #374151 !important; }
    html[data-skin="dark"] .text-dark,
    html[data-skin="dark"] .text-muted { color: #ffffff !important; }
    html[data-skin="dark"] .filter-dropdown .dropdown-menu { background: #0f172a !important; border-color: #1f2937 !important; }
    html[data-skin="dark"] .filter-dropdown .form-check-label { color: #ffffff !important; }
    html[data-skin="dark"] .btn-outline-gold { background: #1e293b !important; border-color: #475569 !important; color: #fbbf24 !important; }
    html[data-skin="dark"] .btn-outline-gold:hover { background: #0f172a !important; border-color: #fbbf24 !important; color: #ffffff !important; }
    html[data-skin="dark"] .modal-content { background: #0f172a !important; border-color: #1f2937 !important; }
    html[data-skin="dark"] .modal-header { background: #0f172a !important; border-color: #1f2937 !important; color: #ffffff !important; }
    html[data-skin="dark"] .modal-title,
    html[data-skin="dark"] .modal-body { color: #ffffff !important; }
    html[data-skin="dark"] #filePreviewModal .modal-header { background: #0f172a !important; color: #ffffff !important; }
    html[data-skin="dark"] .pill-wfo { background: #1e3a5f !important; color: #93c5fd !important; }
    html[data-skin="dark"] .pill-wfh { background: #052e16 !important; color: #4ade80 !important; }
    html[data-skin="dark"] .pill-izin { background: #431407 !important; color: #fdba74 !important; }
    html[data-skin="dark"] .pill-sakit { background: #450a0a !important; color: #f87171 !important; }
    html[data-skin="dark"] .late-tag { background: #450a0a !important; color: #f87171 !important; }
    html[data-skin="dark"] [id^="keteranganModal-"] .modal-body,
    html[data-skin="dark"] [id^="keteranganModal-"] .modal-body div,
    html[data-skin="dark"] [id^="keteranganModal-"] .modal-body div[style*="var(--text-main)"],
    html[data-skin="dark"] [id^="keteranganModal-"] .modal-body .text-dark {
...
<div class="modern-card">
    ...
    @endif
            ...
            <div class="filter-section">
                <form method="GET" action="{{ route('absensi.peserta.index') }}" id="filterForm" class="d-flex flex-wrap align-items-center gap-3">
                    <!-- Filter Tanggal -->
                    <div class="d-flex align-items-center gap-2">
                        <label class="form-label fw-bold small text-muted mb-0"><i class="ri-calendar-line"></i> Tanggal Mulai</label>
                        <input type="date" name="start_date" class="form-control form-control-custom" value="{{ request('start_date') }}" onclick="this.showPicker()" style="width: auto; min-width: 160px;">
                    </div>
                    <div class="d-flex align-items-center gap-2">
                        <label class="form-label fw-bold small text-muted mb-0"><i class="ri-calendar-check-line"></i> Tanggal Selesai</label>
                        <input type="date" name="end_date" class="form-control form-control-custom" value="{{ request('end_date') }}" onclick="this.showPicker()" style="width: auto; min-width: 160px;">
                    </div>
                    <button type="submit" name="filter_date" value="1" class="btn-filter-apply"><i class="ri-calendar-check-line"></i> Terapkan Tanggal</button>
                    @if(request()->has('start_date') || request()->has('end_date'))
                        <a href="{{ route('absensi.peserta.index') }}?{{ http_build_query(request()->except(['start_date', 'end_date', 'page'])) }}" class="btn-filter-reset">
                            <i class="ri-close-line"></i> Reset Tanggal
                        </a>
                    @endif

                    <!-- Filter Jenis Kehadiran -->
                    ...
                    <button type="submit" name="filter_category" value="1" class="btn-filter-apply"><i class="ri-filter-3-line"></i> Terapkan Filter</button>
                    @if(request()->has('filter_jenis_kehadiran'))
                        <a href="{{ route('absensi.peserta.index') }}?{{ http_build_query(request()->except(['filter_jenis_kehadiran', 'page'])) }}" class="btn-filter-reset">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="fw-bold text-dark">{{ $val->peserta->nama ?? '-' }}</td>
