...
@section('title', 'Edit Broadcast Email | Admin - SMART Setjen DPR RI')
@push('styles')
...
<div class="content-wrapper">
    <div class="container-fluid px-md-5">
        <div class="page-title-area">
            ...
            <h1 class="page-title">{{ $isWeb ? 'Ubah Pengumuman Web' : 'Ubah Broadcast Email' }}</h1>
        </div>
        ...
        <div class="broadcast-card mb-5">
            <div class="broadcast-card-header d-flex flex-wrap justify-content-between align-items-center gap-2">
                <div>
                    ...
                    <div class="text-muted" style="font-size:.86rem;">
                        @if($isWeb)
                            Perbarui isi sebelum diterbitkan ulang (hanya jika belum pernah diterbitkan).
                        @else
                            Perbarui isi dan target, lalu kirim ulang (hanya untuk broadcast yang belum ada email terkirim).
                        @endif
                    </div>
                </div>
                @if(! $isWeb)
            ...
            <div class="broadcast-body">
                <form method="POST" action="{{ route($rp.'.update', $broadcast) }}" id="broadcast-form" data-no-global-loader>
                    @csrf
                    @method('PUT')
                    @include('admin.broadcast-email._form_fields', [
                        'broadcast' => $broadcast,
                        'pesertaOptions' => $pesertaOptions,
                        'submitLabel' => $isWeb ? 'Simpan & terbitkan ulang' : 'Simpan & Kirim Ulang',
