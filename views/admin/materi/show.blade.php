...
@section('title', 'Detail Materi | Admin - SMART Setjen DPR RI')
@section('content')
...
<style>
    ...
    /* Ukuran tombol Edit/Kembali (header) dibuat lebih kecil */
    .materi-actions .btn {
...
<div class="content-wrapper">
    <div class="container-fluid px-md-5">
        <div class="page-title-area d-flex justify-content-between align-items-center flex-wrap gap-2">
            ...
            <div class="materi-actions d-flex gap-2">
                <a href="{{ route('materi.edit', $data->id) }}" class="btn btn-primary" style="border-radius: 12px;">
                    <i class="ri-edit-2-line"></i> Edit
                </a>
                <a href="{{ route('materi.index') }}" class="btn btn-light" style="border-radius: 12px;">
        ...
        <div class="modern-card p-4">
            <div class="d-flex justify-content-between flex-wrap gap-3 mb-1">
                <h1 class="fw-800 mb-0">{{ $data->judul }}</h1>
                @if(!empty($data->navbar_nama))
                    <div class="text-muted fw-700 d-flex align-items-center gap-1">
                        <h4> 
                            Menu: &nbsp;
                            @if($data->navbar_icon)
                                <i class="{{ $data->navbar_icon }}"></i>
                            @endif
                            <span>&nbsp;{{ $data->navbar_nama }}</span>
                        </h4>
                    </div>
                @endif
            </div>
            <div class="d-flex justify-content-between align-items-center mt-2 mb-1">
                <div class="materi-meta">
                    <span class="meta-pill">
                        <i class="ri-building-4-line"></i> {{ $data->satker->nama ?? 'Semua Unit' }}
                    </span>
                    <span class="meta-pill {{ $data->is_publish ? 'pill-publish' : 'pill-draft' }}">
                        <i class="ri-upload-2-line"></i> {{ $data->is_publish ? 'Published' : 'Draft' }}
                    </span>
                    <span class="meta-pill {{ $data->status ? 'pill-aktif' : 'pill-nonaktif' }}">
                        <i class="ri-shield-check-line"></i> {{ $data->status ? 'Aktif' : 'Nonaktif' }}
                    </span>
                ...
                <div class="d-flex align-items-center gap-1 text-muted">
                    ...
                    <span>{{ $data->tanggal_publish->locale('id')->translatedFormat('d F Y') }}</span>
                </div>
            </div>
                    ...
                    <div class="file-card">
                        ...
                        @if($data->file_materi)
                            <div class="file-actions">
                                <a class="btn-file primary" href="{{ route('materi.admin.download', [$data->id, 'file']) }}">
                        ...
                        @if($data->pdf_materi)
                            <div class="file-actions">
                                <a class="btn-file" href="javascript:void(0)"
                                   onclick="openPdfViewer('{{ route('materi.admin.pdf.view', $data->id) }}', '{{ route('materi.admin.download', [$data->id, 'pdf']) }}')">
                                    <i class="ri-eye-line"></i> Lihat
                                </a>
                                <a class="btn-file primary" href="{{ route('materi.admin.download', [$data->id, 'pdf']) }}">
                                    <i class="ri-download-2-line"></i> Download
                                </a>
                            </div>
                        @else
