...
@section('title', 'Buat Lowongan | Admin - SMART Setjen DPR RI')
@section('content')
@push('styles')
...
<div class="content-wrapper">
    <div class="container-fluid px-md-5">
        <div class="page-title-area">
            ...
            <h1 class="page-title" style="font-size: 1.75rem;">Tambah Lowongan Baru</h1>
        </div>
        ...
        <div class="modern-card my-4">
            <form action="{{ route('lowongan.store') }}" method="POST" enctype="multipart/form-data" id="form-lowongan-create">
                @csrf
                <div class="row">
                    <div class="col-md-8">
                        <div class="mb-4">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="hidden" name="detail" id="detail" value="{{ old('detail') }}">
                            @error('detail')
                    ...
                    <div class="col-md-4">
                        <div class="mb-4">
                            ...
                            <select class="form-select @error('id_satker') is-invalid @enderror"
                                ...
                                @foreach($satker as $uk)
                                    <option value="{{ $uk->id }}" {{ old('id_satker', $lockedSatkerId ?? null) == $uk->id ? 'selected' : '' }}>
                                        {{ $uk->nama }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" class="form-control @error('jumlah_posisi') is-invalid @enderror"
                                   id="jumlah_posisi" name="jumlah_posisi" value="{{ old('jumlah_posisi') }}" min="1">
                            @error('jumlah_posisi')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>

                        @php $oldJenis = (array) old('jenis_pendidikan', []); @endphp
                        <div class="mb-4">
                        ...
                        <div class="mb-4">
                            ...
                            <select class="form-select @error('status') is-invalid @enderror" 
                                ...
                                <option value="9" {{ old('status') == '9' ? 'selected' : '' }}>Tidak Aktif</option>
                            </select>
                            @error('status')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
    ...
    if (form) {
        form.addEventListener('submit', function(e) {
            ...
            Swal.fire({
                ...
                text: 'Apakah Anda yakin data lowongan ini sudah benar dan ingin disimpan?',
                icon: 'question',
