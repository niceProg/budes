...
<head>
    <meta charset="UTF-8">
    <title>Verifikasi Login - {{ config('app.name') }}</title>
    ...
    <style>
        ...
        .tf-status {
            font-size: 0.8rem;
            color: #22c55e; /* hijau muda lebih terang */
            margin-bottom: 10px;
        }
        .tf-button {
            width: 100%;
        ...
        /* === Dark mode untuk halaman OTP === */
        html[data-skin="dark"] body {
            background: #020617;
            color: #e5e7eb;
        }
        html[data-skin="dark"] .bg-pattern {
            opacity: 0.03;
        }
        html[data-skin="dark"] .bg-pattern {
            opacity: 0.03;
        }
        html[data-skin="dark"] .tf-panel {
            background: #020617;
            border-color: #1f2937;
            box-shadow: 0 20px 40px rgba(0,0,0,0.7);
        }
        html[data-skin="dark"] .tf-title {
            color: #ffffff;
        }
        html[data-skin="dark"] .tf-subtitle,
        html[data-skin="dark"] .tf-label,
        html[data-skin="dark"] .tf-helper,
        html[data-skin="dark"] .tf-resend {
            color: #e5e7eb;
        }
        html[data-skin="dark"] .tf-email {
            color: #ffffff;
        }
        html[data-skin="dark"] .tf-status {
            color: #86efac; /* hijau muda sangat terang di dark mode */
        }
        html[data-skin="dark"] .tf-otp {
            background: #020617;
            border-color: #374151;
            color: #fbbf24;
        }
        html[data-skin="dark"] .tf-otp:focus {
            border-color: #fbbf24;
            box-shadow: 0 0 0 1px #fbbf24;
        }
        html[data-skin="dark"] .tf-button {
            background: linear-gradient(135deg, #facc15, #b45309);
            color: #020617;
        }
        html[data-skin="dark"] .tf-back {
            background: rgba(15,23,42,0.9);
            border-color: #1f2937;
            color: #e5e7eb;
        }
        html[data-skin="dark"] .tf-back:hover {
            background: #020617;
        }
        html[data-skin="dark"] .tf-resend button {
            color: #fbbf24;
        }
        html[data-skin="dark"] .tf-lockout {
            background: #1f0a0a;
            border-color: #7f1d1d;
            color: #fca5a5;
        }
    ...
    <script>
        // Terapkan mode yang tersimpan (dark / light) juga di halaman two-factor
        (function() {
            try {
...
<body>
    ...
    <a href="{{ route('user.twofactor.method') }}" class="tf-back">
        <span>&larr;</span><span>Kembali</span>
    </a>
    <div class="tf-container">
        <div class="tf-title">Selamat datang di {{ config('app.name') }}</div>
        <div class="tf-subtitle">Masukkan kode yang kami kirim ke email Anda untuk menyelesaikan login.</div>

        <div class="tf-panel">
            <div class="tf-label">Kode dikirim ke</div>
            <div class="tf-email">{{ $email }}</div>

            @if (session('status'))
                <div class="tf-status">{{ session('status') }}</div>
            @endif
            ...
            <!-- Form Verifikasi -->
            <form method="POST" action="{{ route('user.twofactor.verify') }}" id="twofactor-form" data-no-page-loader>
                @csrf
                ...
                <div class="tf-helper">Kode berlaku 10 menit. Maksimal 10 kali percobaan, lalu tunggu 30 menit.</div>

                <button type="submit" class="tf-button" id="btnVerify" {{ !empty($isLockedOut) ? 'disabled style="opacity:.5;cursor:not-allowed;"' : '' }}>
            ...
            <!-- Form Resend -->
            <form method="POST" action="{{ route('user.twofactor.resend') }}" class="tf-resend" id="resend-form">
                @csrf
        ...
        (function() {
            ...
            form.addEventListener('submit', (e) => {
                ...
                if (hidden.value.length !== 6) {
                    ...
                    // show global loader manually (since this form disables auto loader)
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (resendForm) {
                resendForm.addEventListener('submit', function() {
                    ...
                    const loadingEl = btnResend.querySelector('.btn-loading');
