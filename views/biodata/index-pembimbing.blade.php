...
@section('title', 'Biodata Peserta | Pembimbing - SMART Setjen DPR RI')
@section('content')
        ...
        <div class="page-title-area">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-1">
                    <li class="breadcrumb-item">Menu Pembimbing</li>
                    <li class="breadcrumb-item active">Biodata Peserta</li>
                </ol>
            </nav>
            <h1 class="page-title">Monitoring Peserta Bimbingan</h1>
            <div class="text-muted small">
                Hanya menampilkan peserta yang dibimbing oleh <strong>{{ $guru->nama }}</strong>
            </div>
        </div>
        ...
        <div class="modern-card">
                    ...
                    <form method="GET" action="{{ request()->url() }}" class="search-container" id="searchForm">
                        <div class="input-group">
                            ...
                            <input type="text" name="search" id="search" class="form-control shadow-none"
                                   placeholder="Cari nama / email / institusi / jurusan..."
            ...
            <div class="filter-section d-flex flex-wrap align-items-center gap-3">
                <form method="GET" action="{{ request()->url() }}" id="filterForm" class="d-flex flex-wrap align-items-center gap-3 flex-grow-1">
                        ...
                        <ul class="dropdown-menu p-2" style="min-width: 180px;">
                            ...
                            <li>
                                <div class="form-check">
                                    ...
                                    <label class="form-check-label w-100" for="stat_3">Selesai</label>
                                </div>
                            </li>
                        </ul>
                    </div>
                    ...
                    @if(request()->has('filter_kategori') || request()->has('filter_status'))
                        <a href="{{ request()->url() }}?{{ http_build_query(request()->except(['filter_kategori', 'filter_status', 'page'])) }}" class="btn-filter-reset">
                        ...
                        @forelse ($data as $index => $val)
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    ...
                                    @elseif($val->status == 3 || $val->status == 0)
                                        <span class="status-badge status-selesai">Selesai Magang</span>
                                    @elseif($val->status == 9)
                                        <span class="status-badge status-selesai">Selesai / BANNED</span>
                                    @endif
                                </td>
                                <td class="text-end">
                                    <a href="{{ route('pembimbing.biodata.show', $val->id) }}" class="btn-action" title="Detail Peserta">
            ...
            <div class="footer-container">
                <div class="d-flex d-flex flex-column flex-md-row justify-content-between align-items-center gap-4">
                    <div class="text-muted fw-700" style="font-size: 0.85rem;">
