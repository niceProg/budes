...
@section('title', 'Detail Biodata Peserta | Atasan - SMART Setjen DPR RI')
@section('content')
    <style>
        :root {
            --primary-dark: #0f172a;
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <div class="d-flex flex-wrap gap-2 align-items-center">
            @php
                $atasanBackUrl = request('from') === 'lulus' ? route('atasan.biodata.lulus.index') : route('atasan.biodata.index');
            @endphp
            <a href="{{ $atasanBackUrl }}" class="btn btn-sm flex-shrink-0 btn-kembali-gold text-decoration-none">
    ...
    @php
        $nodinIncomplete = empty($data->nodin_1) || empty($data->nodin_2);
    @endphp
                        ...
                        @if($data->id_satker)
                            <div>
                                ...
                                <span class="badge badge-gold-light">{{ $data->satker->kode ?? '-' }}</span>
                                    ...
                                    <i class="ri-user-star-line"></i> {{ $data->id_mentor ? 'Ubah Mentor' : 'Atur Mentor' }}
                                </button>
                                </div>
                                @if($data->id_mentor && $data->mentor)
                                    <div class="mt-2">
    ...
    <div class="card">
        <div class="card-header">
            <h6><i class="ri-file-list-3-line"></i> Nota Dinas (Nodin)</h6>
        </div>
        <div class="card-body">
            @php
                $urlNodin1 = $data->nodin_1 ? (str_contains($data->nodin_1, 'nodin_1/') ? file_url($data->nodin_1) : file_url('nodin_1/' . $data->nodin_1)) : null;
                $urlNodin2 = $data->nodin_2 ? (str_contains($data->nodin_2, 'nodin_2/') ? file_url($data->nodin_2) : file_url('nodin_2/' . $data->nodin_2)) : null;
                $urlNotaSelesai = $data->nota_dinas_selesai ? file_url('nota_dinas_selesai/' . $data->nota_dinas_selesai) : null;
            @endphp

            <div class="row">
                <div class="col-md-4 mb-3">
                    <div class="doc-item {{ $urlNodin1 ? 'nodin-filled' : 'nodin-empty' }}">
                        <span class="doc-label">Lembar Persetujuan (Nodin 1)</span>
                        <div class="btn-group">
                            @if($urlNodin1)
                                <button type="button" class="btn btn-sm btn-outline-primary btn-preview-file" data-url="{{ $urlNodin1 }}" data-title="Lembar Persetujuan (Nodin 1)">
                ...
                <div class="col-md-4 mb-3">
                    <div class="doc-item {{ $urlNodin2 ? 'nodin-filled' : 'nodin-empty' }}">
                        <span class="doc-label">Nodin Satuan Kerja (Nodin 2)</span>
                        <div class="btn-group">
                            @if($urlNodin2)
                                <button type="button" class="btn btn-sm btn-outline-primary btn-preview-file" data-url="{{ $urlNodin2 }}" data-title="Nodin Satuan Kerja (Nodin 2)">
                                    <i class="ri-eye-line"></i>
                                </button>
                            @else
                                <span class="text-muted small ms-2">File Tidak Tersedia</span>
                ...
                <div class="col-md-4 mb-3">
                    <div class="doc-item {{ $urlNotaSelesai ? 'nodin-filled' : 'nodin-empty' }}">
                        <span class="doc-label">Nota Dinas Selesai</span>
                        <div class="btn-group">
                            @if($urlNotaSelesai)
                                <button type="button" class="btn btn-sm btn-outline-primary btn-preview-file" data-url="{{ $urlNotaSelesai }}" data-title="Nota Dinas Selesai">
                                    <i class="ri-eye-line"></i>
                                </button>
                            @else
                                <span class="text-muted small ms-2">File Tidak Tersedia</span>
    ...
    $(document).ready(function() {
        ...
        // Logic untuk Atasan: hanya bisa ubah mentor, satuan kerja fixed
        $(document).on('click', '.btn-edit-satker', function(e) {
            ...
            // Ambil id_satker atasan dari session
            @php
                $atasanSatkerId = null;
                if (session('auth_type') === 'pegawai') {
                    $roleAs = session('role_as');
                    $atasanSatkerId = $roleAs['id_satker'] ?? null;
                }
            @endphp
            const atasanSatkerId = @json($atasanSatkerId);

            // Filter satuan kerja hanya milik atasan
            const filteredUnitKerja = satkerData.filter(uk => uk.id == atasanSatkerId);

            let satkerOptions = '<div class="mb-3"><label class="form-label fw-bold">Satuan Kerja</label><select id="swal-unit-kerja-select" class="form-select" style="width: 100%; background-color: #f8f9fa; cursor: not-allowed;" disabled>';
            if (filteredUnitKerja.length > 0) {
                const uk = filteredUnitKerja[0];
                satkerOptions += `<option value="${uk.id}" selected>${satkerLabel(uk)}</option>`;
            } else {
                satkerOptions += '<option value="">Satuan kerja tidak ditemukan</option>';
            }
            satkerOptions += '</select></div>';
            satkerOptions += '<small class="text-muted d-block mb-2"><i class="ri-information-line"></i> Satuan kerja tidak dapat diubah</small>';

            let mentorOptions = '<div class="mb-2"><label class="form-label fw-bold">Mentor</label><select id="swal-mentor-select" class="form-select" style="width: 100%;">';
            mentorOptions += '<option value="">-- Pilih Mentor --</option>';
            ...
            Swal.fire({
                ...
                didOpen: () => {
                    ...
                    if (atasanSatkerId) {
                        loadMentors(atasanSatkerId, currentMentorId);
                    }
                },
                preConfirm: () => {
                    const unitId = atasanSatkerId; // Selalu gunakan satuan kerja atasan
                    const mentorId = $('#swal-mentor-select').val();

                    if (!unitId) {
                        Swal.showValidationMessage('Satuan kerja tidak ditemukan');
                        return false;
                    }
