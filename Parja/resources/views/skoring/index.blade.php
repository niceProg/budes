...
<div class="d-sm-flex align-items-center justify-content-between mb-4 ">
    ...
    <div>
        @if (rbac_can_create_parja())
            <a href="{{ route('parja.skoring.add') }}" class="btn btn-success">
...
<div class="card ">
    <div class="card-body">
            ...
            <table id="datatable" class="table table-striped table-bordered">
                <thead>
                    <tr>
                        ...
                        <th scope="col" class="p-1 text-center" style="width: 50%;">Skoring</th>
                        <th scope="col" class="p-1 text-center" style="width: 25%;">Aksi</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($skorings as $skoring)
                        <tr>
                            <td class="text-center">{{ $skorings->firstItem() + $loop->index }}</td>
                            <td>{{ $skoring->skoring }}</td>
                            <td class="text-center">
                                @if (rbac_can_edit_parja())
                                    <a href="{{ route('parja.skoring.edit', $skoring->id) }}" class="btn btn-primary">
                                        <i class="ri-edit-2-line"></i> Edit
                                    </a>
                                @endif
                                @if (rbac_can_delete_parja())
                                    <form action="{{ route('parja.skoring.destroy', $skoring->id) }}" method="POST" style="display: inline-block" class="form-hapus">
        ...
        @if($skorings->hasPages())
        <div class="d-flex justify-content-between align-items-center mt-3 flex-wrap gap-2">
            <p class="text-muted small mb-0">Menampilkan {{ $skorings->firstItem() }}–{{ $skorings->lastItem() }} dari {{ $skorings->total() }} data</p>
            {{ $skorings->appends(request()->only(['search']))->links() }}
        </div>
        @endif
    </div>
</div>
