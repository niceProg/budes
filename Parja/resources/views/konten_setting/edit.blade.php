...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            ...
            <form action="{{ route('parja.konten_setting.update', $record->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="mb-3">
                    <label for="nama" class="form-label fw-bold">Nama <span class="text-danger">*</span></label>
                    <input required type="text" class="form-control" id="nama" name="nama" placeholder="Contoh: alumni_home_banner_1_image" value="{{ $record->nama }}">
                </div>
                <div class="mb-3">
                    <label for="nilai" class="form-label fw-bold">Nilai</label>
                    <textarea class="form-control" id="nilai" name="nilai" rows="4" placeholder="URL gambar / judul / link sesuai nama slot">{{ $record->nilai }}</textarea>
                </div>
                <div class="mb-3">
                    <label for="tipe" class="form-label fw-bold">Tipe</label>
                    <input type="text" class="form-control" id="tipe" name="tipe" placeholder="text / image / url / dll" value="{{ $record->tipe }}" maxlength="20">
                </div>
