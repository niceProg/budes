...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    ...
    <div>
        @if (rbac_can_create_parja())
            <a href="{{ route('parja.publikasi.add') }}" class="btn btn-success">
                <i class="ri-pencil-line"></i> Tambah Publikasi
            </a>
        @endif
    </div>
</div>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                @if (rbac_can_edit_parja())
                                    <a href="{{ route('parja.publikasi.edit', $item->id) }}" class="btn btn-primary">
                                        <i class="ri-edit-2-line"></i> Edit
                                    </a>
                                @endif
                                @if (rbac_can_delete_parja())
                                    <form action="{{ route('parja.publikasi.destroy', $item->id) }}" method="POST" style="display: inline-block" class="form-hapus">
                                        @csrf
                                        @method('DELETE')
