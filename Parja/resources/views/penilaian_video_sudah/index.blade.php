...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <div>
        <ol class="breadcrumb fs-sm mb-1">
            ...
            <li class="breadcrumb-item active" aria-current="page">Daftar Penilaian Video (Sudah)</li>
        </ol>
        <h4 class="main-title mb-0">Daftar Peserta Sudah Dinilai Video</h4>
    </div>
    <div>
        @if (rbac_can_create_parja())
            <a href="{{ route('parja.penilaian_video.add') }}" class="btn btn-success">
                <i class="ri-pencil-line"></i> Tambah Penilaian Video
            </a>
        @endif
    </div>
</div>
                    ...
                    @foreach ($penilaians as $penilaian)
                        <tr>
                            ...
                            <td class="text-center fw-bold">
                                @if ($penilaian->nilai_video !== null)
                                    <span class="badge {{ $penilaian->nilai_video >= 70 ? 'bg-success' : 'bg-danger' }}">
                                        {{ number_format($penilaian->nilai_video, 2) }}
                                    </span>
                                @else
                                    -
                                @endif
                            </td>
                            <td>{{ $penilaian->catatan_video ?? '-' }}</td>
                            ...
                            <td class="text-center">
                                @if (rbac_can_edit_parja())
                                    <a href="{{ route('parja.penilaian_video.edit', $penilaian->id) }}" class="btn btn-primary btn-sm">
                                        <i class="ri-edit-2-line"></i> Edit
                                    </a>
                                @endif
                                @if (rbac_can_delete_parja())
                                    <form action="{{ route('parja.penilaian_video.destroy', $penilaian->id) }}" method="POST"
    ...
    $(document).ready(function() {
        $('.btn-hapus').on('click', function(e) {
            ...
            Swal.fire({
                ...
                text: 'Data penilaian video yang dihapus tidak dapat dikembalikan!',
                icon: 'warning',
