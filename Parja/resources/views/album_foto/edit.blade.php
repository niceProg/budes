...
@section('content')
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <div>
            ...
            <h4 class="main-title mb-0">Edit Album Foto</h4>
        </div>
    </div>

    <div class="card">
        <div class="card-body">
            <form action="{{ route('parja.album_foto.update', $record->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="mb-3">
                    <label for="tanggal" class="form-label fw-bold">Tanggal <span class="text-danger">*</span></label>
                    <input required type="date" class="form-control" id="tanggal" name="tanggal" value="{{ $record->tanggal }}">
                </div>
                <div class="mb-3">
                    <label for="album_foto" class="form-label fw-bold">Album Foto <small class="text-muted">(JSON array nama file)</small></label>
                    <textarea class="form-control" id="album_foto" name="album_foto" rows="4" placeholder="[]">{{ $record->album_foto }}</textarea>
                </div>
