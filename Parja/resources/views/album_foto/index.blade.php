...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    ...
    <div>
        @if (rbac_can_create_parja())
            <a href="{{ route('parja.album_foto.add') }}" class="btn btn-success">
                <i class="ri-pencil-line"></i> Tambah Album Foto
            </a>
        @endif
    </div>
</div>
            ...
            <table id="datatable" class="table table-striped table-bordered">
                <thead>
                    <tr>
                        ...
                        <th class="p-1 text-center" style="width: 35%;">Deskripsi</th>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                @if (rbac_can_edit_parja())
                                    <a href="{{ route('parja.album_foto.edit', $item->id) }}" class="btn btn-primary">
                                        <i class="ri-edit-2-line"></i> Edit
                                    </a>
                                @endif
                                @if (rbac_can_delete_parja())
                                    <form action="{{ route('parja.album_foto.destroy', $item->id) }}" method="POST" style="display: inline-block" class="form-hapus">
                                        @csrf
                                        @method('DELETE')
