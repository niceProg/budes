...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <div>
        <ol class="breadcrumb fs-sm mb-1">
            ...
            <li class="breadcrumb-item active" aria-current="page">Daftar Penilaian CV (Sudah)</li>
        </ol>
        <h4 class="main-title mb-0">Daftar Peserta Sudah Dinilai CV</h4>
    </div>
    <div>
        @if (rbac_can_create_parja())
            <a href="{{ route('parja.penilaian_cv.add') }}" class="btn btn-success">
                <i class="ri-pencil-line"></i> Tambah Penilaian CV
            </a>
        @endif
    </div>
</div>
            ...
            <table id="datatable" class="table table-striped table-bordered">
                <thead>
                    <tr>
                        ...
                        <th class="p-1 text-center">Dinilai Oleh</th>
                    ...
                    @foreach ($penilaians as $penilaian)
                        <tr>
                            ...
                            <td class="text-center fw-bold">
                                @if ($penilaian->nilai_cv !== null)
                                    <span class="badge {{ $penilaian->nilai_cv >= 70 ? 'bg-success' : 'bg-danger' }}">
                                        {{ number_format($penilaian->nilai_cv, 2) }}
                                    </span>
                                @else
                                    -
                                @endif
                            </td>
                            <td>{{ $penilaian->catatan_cv ?? '-' }}</td>
                            ...
                            <td class="text-center">
                                @if (rbac_can_edit_parja())
                                    <a href="{{ route('parja.penilaian_cv.edit', $penilaian->id) }}" class="btn btn-primary btn-sm">
                                        <i class="ri-edit-2-line"></i> Edit
                                    </a>
                                @endif
                                @if (rbac_can_delete_parja())
                                    <form action="{{ route('parja.penilaian_cv.destroy', $penilaian->id) }}" method="POST"
    ...
    $(document).ready(function() {
        $('.btn-hapus').on('click', function(e) {
            ...
            Swal.fire({
                ...
                text: 'Data penilaian CV yang dihapus tidak dapat dikembalikan!',
                icon: 'warning',
