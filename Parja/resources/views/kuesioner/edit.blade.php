...
<div class="d-flex align-items-center justify-content-between mb-4">
    <div>
        <h4 class="main-title mb-0">Edit Pertanyaan</h4>
...
<div class="card card-one">
    ...
    <div class="card-body">
        <form action="{{ route($routePrefix . 'kuesioner.update', [$survei->id, $kuesioner->id]) }}" method="POST"
            ...
            <div class="mb-3">
                ...
                <select name="tipe_jawaban" class="form-select @error('tipe_jawaban') is-invalid @enderror" required>
                    <option value="pilihan" {{ old('tipe_jawaban', $kuesioner->tipe_jawaban) === 'pilihan' ? 'selected' : '' }}>Pilihan Ganda</option>
                    <option value="isian" {{ old('tipe_jawaban', $kuesioner->tipe_jawaban) === 'isian' ? 'selected' : '' }}>Isian Teks</option>
                    <option value="skala" {{ old('tipe_jawaban', $kuesioner->tipe_jawaban) === 'skala' ? 'selected' : '' }}>Skala Rating</option>
                    <option value="checkbox" {{ old('tipe_jawaban', $kuesioner->tipe_jawaban) === 'checkbox' ? 'selected' : '' }}>Checkbox</option>
                </select>
            ...
            <div class="mb-4">
                ...
                <textarea name="deskripsi" rows="2" class="form-control">{{ old('deskripsi', $kuesioner->deskripsi) }}</textarea>
            </div>
